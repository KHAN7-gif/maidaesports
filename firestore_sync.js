...
/* ── WRITE (create or overwrite document) ──
   PATCH with updateMask=json: Firestore creates the doc if it doesn't
   exist, or replaces just the "json" field if it does. */
function fsSet(path, data, onOk, onErr){
  var url = _FS.base + "/" + path + "?updateMask.fieldPaths=json";
  fetch(url, {
    method: "PATCH",
...
function fsGet(path, cb, onErr){
  ...
  fetch(url)
  .then(function(r){
    _FS.ok = true; // connection itself succeeded, even if doc not found
...
function fsCheckStatus(cb){
  var url = _FS.base + "/maida_ping?updateMask.fieldPaths=json";
  fetch(url, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
