...
Sistem pengurusan turnamen PUBG Mobile esports secara live, dengan sync realtime antara semua peranti (desktop & mobile) melalui Firebase Realtime Database.
...
| `firebase_sync.js` | Modul sync Firebase Realtime Database (REST API) — wajib untuk semua fail |
...
Admin boleh tambah/edit/padam pengguna dari panel **Pengurusan Crew** dalam `index.html`. Senarai pengguna sync ke semua peranti melalui Firebase.

---

## 🔥 Firebase Realtime Database Setup (Sekali Sahaja)

**PERCUMA — TANPA KAD KREDIT** (berbeza dengan Cloud Firestore)

1. Buka [console.firebase.google.com](https://console.firebase.google.com) → project **maida-esports**
2. **Build → Realtime Database → Create Database**
3. Pilih lokasi (cth: `asia-southeast1`)
...
```
5. **Enable**
6. Catat URL database yang dipaparkan (format: `https://maida-esports-default-rtdb.<region>.firebasedatabase.app`)
7. Pastikan `DB_URL` dalam `firebase_sync.js` sepadan dengan URL ini
...
> **Nota:** Logo/banner team **TIDAK** disync (terlalu besar). Setiap peranti kekalkan logo tempatan; nama team, ranking, dan skor sahaja yang disync merentas peranti.
...
leaderboard_FINAL.html (input PTS/ELIMS/P1-P4)
        │  fsSet → maida/liveData (Firebase, debounced 300ms)
        ▼
   ┌─────────────┬──────────────┬───────────────┐
   ▼             ▼              ▼               ▼
popup_kill   OBS Leaderboard   public_results  index.html (status)
(fsListen 1s)(fsListen 1s)     (fsListen 1s)    (fsListen 2s)
```

Semua peranti sync melalui Firebase (polling 1-3 saat), dengan **localStorage** sebagai cache/fallback tempatan.
...
- `spDebug()` — banding data tempatan vs Firebase
- `spForceSync()` — push data tempatan ke Firebase
- `spForceLoad()` — tarik data dari Firebase ke tempatan

Badge di sudut kanan atas menunjukkan status sambungan Firebase.
